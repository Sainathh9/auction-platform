...
const TEST_AUCTION_ID = `auc_load_${Date.now()}`;
...
async function runLoadTest() {
    ...
    // 1. Seed the auction
    console.log(`\n[Step 1] Initializing Auction ${TEST_AUCTION_ID}...`);
    try {
        const seedPayload = {
            auctionId: TEST_AUCTION_ID,
            title: "Load Testing Masterpiece",
        ...
        if (!response.ok) throw new Error(data.error || 'Seeding failure');
